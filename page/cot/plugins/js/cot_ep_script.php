...
document.getElementById('exportReqBtn').addEventListener('click', () => {
    ...
    const url = `../../process/cot_ep_export_data.php${searchTerm ? '?search=' + encodeURIComponent(searchTerm) : ''}`;
    window.location.href = url;
});

function loadTableData(offset, limit, search = '') {
    fetch(`../../process/cot_ep_get_data.php?offset=${offset}&limit=${limit}&search=${encodeURIComponent(search)}`)
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
