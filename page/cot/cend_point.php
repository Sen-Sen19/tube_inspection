            ...
            <option value="Out of Tolerance">Out of Tolerance</option>
                            ...
                            <div id="accounts_table_res" class="table-responsive"
                                ...
                                <table id="sp_cotdb" class="table table-sm table-head-fixed text-nowrap table-hover">
                                    ...
                                    <tbody id="ep_cotdb_body" style="text-align: center; padding:20px;">
...
function loadTableData(offset, limit, reset = false) {
    ...
    const url = `../../process/cot_ep_get_data.php?offset=${offset}&limit=${limit}&partName=${partName}&inspectedBy=${inspectedBy}&defectType=${defectType}&dateFrom=${dateFrom}&dateTo=${dateTo}`;
    ...
    fetch(url)
        ...
        .then(data => {
            if (reset) {
                document.getElementById('ep_cotdb_body').innerHTML = ''; 
            }
            populateTable(data);
...
function exportTable() {
    ...
    const filename = `COT_End_Point_${currentDate}.csv`;

    // Construct the URL with search parameters
    const url = `../../process/cot_ep_export_data.php?partName=${encodeURIComponent(partName)}&inspectedBy=${encodeURIComponent(inspectedBy)}&defectType=${encodeURIComponent(defectType)}&dateFrom=${encodeURIComponent(dateFrom)}&dateTo=${encodeURIComponent(dateTo)}`;
...
function openModalWithData(rowData) {
    ...
    function saveInspectionDetails() {
        ...
        fetch('../../process/ep_cot_update.php', {
            method: 'POST',
            body: formData
        })
            .then(response => {
