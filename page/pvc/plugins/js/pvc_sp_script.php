...
document.getElementById('exportReqBtn').addEventListener('click', () => {
    ...
    const url = `../../process/pvc_sp_export_data.php${searchTerm ? '?search=' + encodeURIComponent(searchTerm) : ''}`;
    window.location.href = url;
});

function loadTableData(offset, limit, search = '') {
    fetch(`../../process/pvc_sp_get_data.php?offset=${offset}&limit=${limit}&search=${encodeURIComponent(search)}`)
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
        ...
        .then(data => {
            ...
            if (offset === 0) {
                document.getElementById('sp_pvcdb_body').innerHTML = ''; // Clear table for new search results
            }
            populateTable(data);
