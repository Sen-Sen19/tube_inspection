...
header('Content-Disposition: attachment; filename="PVC_Start_Point.csv"');
...
$sql = "SELECT * FROM sp_pvcdb";

if (!empty($search)) {
    $stmt = $conn->prepare("SELECT * FROM sp_pvcdb WHERE part_name LIKE ?");
    $searchTerm = "%$search%";
    $stmt->bind_param("s", $searchTerm);
} else {
    $stmt = $conn->prepare("SELECT * FROM sp_pvcdb");
}
